...
export default function ChatPage() {
  ...
  return (
    <main className="flex min-h-[calc(100dvh-7rem)] flex-col gap-4">
      <header>
        <p className="text-caption text-ink-muted-48">{chat.connected ? "SSE conectado" : "Aguardando stream"}</p>
        ...
        <div className="flex-1 space-y-3 overflow-y-auto">
          {chat.messages.length === 0 ? (
            <div className="flex min-h-80 items-center justify-center text-center text-ink-muted-48">
              FinAI - Seu assistente financeiro. Pergunte sobre gastos ou envie um comprovante!
            </div>
          ) : (
